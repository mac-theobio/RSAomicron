...
if (FALSE) {
    predict(fit, newparams = pop_vals[1,], perfect_tests = TRUE, confint = FALSE)
    ensemble <- list()
    ## fixme: skip progress bar if non-interactive?
...
base <- predict(fit, newdata = NULL) |> dplyr::select(prov, time, reinf)
ensemble <- dplyr::bind_cols(base, ensemble)
